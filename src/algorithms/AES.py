from ..helpers.fileHelpers import pickFileFor


Sbox = (
    ,
)

InvSbox = (
    ,
)


xtime = lambda a: (((a << 1) ^ 0x1B) & 0xFF) if (a & 0x80) else (a << 1)


Rcon = (
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
    0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A,
    0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A,
    0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39,
)


def text2matrix(text):
    matrix = []
    for i in range(16):
        byte = (text >> (8 * (15 - i))) & 0xFF
        if i % 4 == 0:
            matrix.append([byte])
        else:
            matrix[i // 4].append(byte)
    return matrix


def matrix2text(matrix):
    text = 0
    for i in range(4):
        for j in range(4):
            text |= (matrix[i][j] << (120 - 8 * (4 * i + j)))
    return text


class AES():

    generateKey = staticmethod(lambda: {'private': {'secret': int.from_bytes(getattr(__import__('os'), 'urandom')(16), byteorder='big')}})

    @staticmethod
    def hashKey(master_key):
        round_keys = text2matrix(master_key)

        for i in range(4, 4 * 11):
            round_keys.append([])
            if i % 4 == 0:
                byte = round_keys[i - 4][0] ^ Sbox[round_keys[i - 1][1]] ^ Rcon[i // 4]
                round_keys[i].append(byte)

                for j in range(1, 4):
                    byte = round_keys[i - 4][j] ^ Sbox[round_keys[i - 1][(j + 1) % 4]]
                    round_keys[i].append(byte)
            else:
                for j in range(4):
                    byte = round_keys[i - 4][j] ^ round_keys[i - 1][j]
                    round_keys[i].append(byte)

        return round_keys

    @staticmethod
    def encrypt(round_keys):
        from progress.bar import FillingCirclesBar as Bar

        srcFile, dstFile = pickFileFor('encrypt')

        with Bar('Processing', max=len(srcFile.read())/(16)+1) as bar:
            srcFile.seek(0)
            while True:
                block = srcFile.read(16)
                if not block: break
                if len(block) != 16: block += ''.join(chr(0) for char in range(16 - len(block))).encode()
                block = int.from_bytes(block, byteorder='big')
                plain_state = text2matrix(block)
                AES.__add_round_key(plain_state, round_keys[:4])
                for i in range(1, 10):
                    AES.__round_encrypt(plain_state, round_keys[4 * i : 4 * (i + 1)])
                AES.__sub_bytes(plain_state)
                AES.__shift_rows(plain_state)
                AES.__add_round_key(plain_state, round_keys[40:])
                cipherBlock = matrix2text(plain_state).to_bytes(16, byteorder='big')
                dstFile.write(cipherBlock)
                bar.next()
        srcFile.close(); dstFile.close()


    @staticmethod
    def decrypt(round_keys):
        from progress.bar import FillingCirclesBar as Bar

        srcFile, dstFile = pickFileFor('decrypt')

        with Bar('Processing', max=len(srcFile.read())/(16)) as bar:
            srcFile.seek(0)
            while True:
                block = srcFile.read(16)
                if not block: break
                block = int.from_bytes(block, byteorder='big')
                cipher_state = text2matrix(block)
                AES.__add_round_key(cipher_state, round_keys[40:])
                AES.__inv_shift_rows(cipher_state)
                AES.__inv_sub_bytes(cipher_state)
                for i in range(9, 0, -1):
                    AES.__round_decrypt(cipher_state, round_keys[4 * i : 4 * (i + 1)])
                AES.__add_round_key(cipher_state, round_keys[:4])
                decryptedBlock = matrix2text(cipher_state).to_bytes(16, byteorder='big')
                if chr(0).encode() in decryptedBlock: decryptedBlock = decryptedBlock.replace('\x00'.encode(), ''.encode())
                dstFile.write(decryptedBlock)
                bar.next()
        srcFile.close(); dstFile.close()



    @staticmethod
    def __add_round_key(s, k):
        for i in range(4):
            for j in range(4):
                s[i][j] ^= k[i][j]


    @staticmethod
    def __round_encrypt(state_matrix, key_matrix):
        AES.__sub_bytes(state_matrix)
        AES.__shift_rows(state_matrix)
        AES.__mix_columns(state_matrix)
        AES.__add_round_key(state_matrix, key_matrix)


    @staticmethod
    def __round_decrypt(state_matrix, key_matrix):
        AES.__add_round_key(state_matrix, key_matrix)
        AES.__inv_mix_columns(state_matrix)
        AES.__inv_shift_rows(state_matrix)
        AES.__inv_sub_bytes(state_matrix)


    @staticmethod
    def __sub_bytes(s):
        for i in range(4):
            for j in range(4):
                s[i][j] = Sbox[s[i][j]]


    @staticmethod
    def __inv_sub_bytes(s):
        for i in range(4):
            for j in range(4):
                s[i][j] = InvSbox[s[i][j]]


    @staticmethod
    def __shift_rows(s):
        s[0][1], s[1][1], s[2][1], s[3][1] = s[1][1], s[2][1], s[3][1], s[0][1]
        s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
        s[0][3], s[1][3], s[2][3], s[3][3] = s[3][3], s[0][3], s[1][3], s[2][3]


    @staticmethod
    def __inv_shift_rows(s):
        s[0][1], s[1][1], s[2][1], s[3][1] = s[3][1], s[0][1], s[1][1], s[2][1]
        s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
        s[0][3], s[1][3], s[2][3], s[3][3] = s[1][3], s[2][3], s[3][3], s[0][3]


    @staticmethod
    def __mix_single_column(a):
        t = a[0] ^ a[1] ^ a[2] ^ a[3]
        u = a[0]
        a[0] ^= t ^ xtime(a[0] ^ a[1])
        a[1] ^= t ^ xtime(a[1] ^ a[2])
        a[2] ^= t ^ xtime(a[2] ^ a[3])
        a[3] ^= t ^ xtime(a[3] ^ u)


    @staticmethod
    def __mix_columns(s):
        for i in range(4):
            AES.__mix_single_column(s[i])


    @staticmethod
    def __inv_mix_columns(s):
        for i in range(4):
            u = xtime(xtime(s[i][0] ^ s[i][2]))
            v = xtime(xtime(s[i][1] ^ s[i][3]))
            s[i][0] ^= u
            s[i][1] ^= v
            s[i][2] ^= u
            s[i][3] ^= v

        AES.__mix_columns(s)